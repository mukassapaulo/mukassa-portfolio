...
export default function Riskflow() {
  return (
    <section id="riskflow" className="relative py-24 md:py-32 border-t border-border-light bg-surface-sub bg-grid">
      <div className="pointer-events-none absolute top-0 left-1/3 w-[500px] h-[500px] rounded-full bg-brand-light blur-[140px]" />
      ...
      <div className="relative mx-auto max-w-6xl px-6 md:px-10">
        ...
        <div className="grid md:grid-cols-[1fr_320px] gap-14 items-start">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-2">
              {STACK.map((s) => (
                <span
                  key={s}
                  className="px-3 py-1 text-xs font-mono text-text-secondary bg-surface border border-border rounded-full"
                >
                  {s}
                </span>
              ))}
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-9 inline-flex items-center gap-2 px-5 py-2.5 bg-brand text-surface text-sm font-medium rounded-full hover:bg-brand-hover transition-colors"
            >
              Ver demonstração
              <IconArrowUpRight />
            </a>
          ...
          <div className="bg-surface border border-border p-6 md:p-8 rounded-card shadow-sm shadow-border/40">
            <RiskMatrix />
